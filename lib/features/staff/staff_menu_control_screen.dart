...
class _StaffMenuControlScreenState extends State<StaffMenuControlScreen> {
  ...
  Future<void> _uploadImageAndFill(TextEditingController imageCtrl, TextEditingController nameCtrl) async {
    try {
      ...
      // Guard against huge images that can cause slow uploads or memory spikes
      if (bytes.lengthInBytes > 2 * 1024 * 1024) {
        throw Exception('Image too large. Please choose a smaller image (<= 2MB).');
      }
...
class _ItemFormDialogState extends State<_ItemFormDialog> {
  ...
  Future<void> _pickAndUpload() async {
    ...
    try {
      ...
      final url = await widget.api.uploadMenuImage(
        fileName: file.name.isEmpty ? 'menu_${DateTime.now().millisecondsSinceEpoch}.jpg' : file.name,
        mime: mime,
        base64: base64,
      );
...
@override
  Widget build(BuildContext context) {
    final previewUrl = (_previewUrl ?? _imageCtrl.text).trim();
    final hasPreview = previewUrl.startsWith('http');
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: OutlinedButton.icon(
                                  ...
                                  label: Text(_uploading ? 'Uploading...' : 'Browse'),
                                ),
                              ),
                            ],
                          ),
                                ...
                                : const Center(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        SizedBox(height: 8),
                                        Text('Uploaded image preview will appear here'),
                                      ],
                                    ),
                                  ),
                          ),
